...
Table~\ref{tab:s1} gives the gross per-run cost on the AVR model. The headline is the cost of software
floating point: kernels with little ``real'' work nonetheless retire hundreds of thousands of
instructions.
...
$\text{cost}=\text{calls}\times\text{cycles/call}$ (Table~\ref{tab:cost}) shows where the time goes and,
crucially, the \emph{operation counts} a hardware datapath would replace.
...
To turn the characterization into a design verdict we plot each kernel on a roofline. Per-invocation
operation count (softfloat calls for fp kernels), bytes moved (load/store count), and cycles are taken
from the two-run difference. The model assumes AVR data-memory bandwidth
$\text{BW}=0.5$~byte/cycle (load/store $\approx$2 cycles/byte), a hardware-arithmetic compute roof of $W$
ops/cycle at SIMD width $W$, and a memory$\leftrightarrow$compute ridge at intensity $I=2W$ ops/byte.
...
Per-invocation instruction counts (i11$-$i1) are model-independent and directly comparable; cycle counts
are not (different CPU models), so we report instructions.
...
the producing ELF archived by content hash. Charts referenced above are emitted as SVG alongside the data
(\code{results/roofline.svg}, \code{results/baseline\_compare.svg}, and per-run op-mix histograms).
